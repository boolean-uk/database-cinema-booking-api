...
describe("Movies Endpoint", () => {
  describe("GET /movies", () => {
    it("will retrieve a list of movies", async () => {
      ...
      const firstMovie = await createMovie("Dodgeball", 120, screen);
      const secondMovie = await createMovie("Scream", 113, screen);

      const tomorrow = new Date()
      tomorrow.setDate(tomorrow.getDate() + 1);

      await createScreening(firstMovie, screen, tomorrow);
      await createScreening(secondMovie, screen, tomorrow);
      ...
      expect(movie1.title).toEqual(firstMovie.title);
      ...
      expect(movie2.title).toEqual(secondMovie.title);
      expect(movie2.runtimeMins).toEqual(113);
      expect(movie2.screenings).not.toEqual(undefined);
      expect(movie2.screenings.length).toEqual(2);
    });
  });
