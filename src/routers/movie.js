...
router.get("/:id", async (req, res) => {
  try {
    ...
    const movie = await theDomainMovie.fetchMovieById(id);

    res
      .status(movie ? 200 : 404)
      .json(movie ? { movie } : { message: "Movie not found" });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const updatedMovie = await theDomainMovie.updateMovie(
      id,
      title,
      runtimeMins
    );
    ...
    res
      .status(updatedMovie ? 201 : 404)
      .json(
        updatedMovie ? { movie: updatedMovie } : { message: "Movie not found" }
      );
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
